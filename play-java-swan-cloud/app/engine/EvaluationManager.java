...
public class EvaluationManager {
    ...
    protected final Map<String, SensorInterface> mSensors = new HashMap<String, SensorInterface>();
    ...
    protected void bindToSensor(final String id,
        ...
        SensorInterface sensor = SensorFactory.getSensor(expression.getEntity());
            ...
            if(sensor!=null) {
                ...
                CowbirdConfiguration.nodeConfiguration().increaseCurrentLoad(1);
            }
...
*/
    ...
    protected void unbindFromSensor(final String id) {
        ...
        if (sensor != null) {
           ...
           // }
            CowbirdConfiguration.nodeConfiguration().decreaseCurrentLoad(1);
        } else {
    ...
    public void stop(String id, Expression expression) {
        ...
        if (expression instanceof LogicExpression) {
            ...
            if(RemoteEvaluationManager.sharedInstance().isEvaluatingExpression(id)) {
                RemoteEvaluationManager.sharedInstance().stopRemotely(id, expression);
            } else {
                stop(id + Expression.LEFT_SUFFIX,
                        ((ComparisonExpression) expression).getLeft());
                stop(id + Expression.RIGHT_SUFFIX,
...
//        }
        ...
        if (!location.equals(Expression.LOCATION_SELF)
            ...
            if(RemoteEvaluationManager.sharedInstance().isEvaluatingExpression(id))
                result = RemoteEvaluationManager.sharedInstance().doRemoteCompare(id, (ComparisonExpression) expression, now);
            else
            ...
            if(RemoteEvaluationManager.sharedInstance().isEvaluatingExpression(id)) {
                /*  Get from remote.    */
                result = RemoteEvaluationManager.sharedInstance().getFromRemote(id, (SensorValueExpression) expression, now);
            }
            else
